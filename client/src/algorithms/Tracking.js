...
async function calculateTransformationCamera(video, startTransformation, previousDescriptor, previousCorners, parameters){
    ...
    for (let i = 0; i < corners.length; i += 2) {
        let point = new DOMPoint(corners[i], corners[i + 1], parameters.fictiveDepth)
    ...
    return {transformation: trans, previousCorners: transformedcorners, previousDescriptor: descriptor}
}

export function calculateTransformation(callback, sensor, video, startMatrix, previousDescriptor, previousCorners, videoParameters){
    calculateTransformationSensors(sensor, startMatrix).then(result => {
        calculateTransformationCamera(
            ...
            result.transformationMatrix.translateSelf(cameraResult.transformation.x, cameraResult.transformation.y)
