    ...
    for(let y = 3; y < grayMatrix.length - 3; y++){
        for(let x = 3; x < grayMatrix[0].length - 3; x++){
            ...
            for(let point in circle)
                point = grayMatrix[point[1]][point[0]];
            let nbOutOfThreshold = 0;

            if(outOfThreshold(intensity, circle[0], threshold))
                nbOutOfThreshold++;
            if(outOfThreshold(intensity, circle[4], threshold))
                nbOutOfThreshold++;
            if(outOfThreshold(intensity, circle[8], threshold))
                nbOutOfThreshold++;
            if(outOfThreshold(intensity, circle[12], threshold))
                nbOutOfThreshold++;
