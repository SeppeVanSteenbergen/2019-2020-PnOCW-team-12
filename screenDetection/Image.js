class Image {
    ...
    countEdges(island) {
        let count = 0;
        let up = this.matrix[island.getMiny()][Math.floor((island.getMaxx()-island.getMinx())/2)];
        let down = this.matrix[island.getMaxy()][Math.floor((island.getMaxx()-island.getMinx())/2)];
        let left = this.matrix[Math.floor((island.getMaxy()-island.getMiny())/2)][island.getMinx()];
        let right = this.matrix[Math.floor((island.getMaxy()-island.getMiny())/2)][island.getMaxx()];
        if(up !== 0) {count++}
        if(down !== 0) {count++}
        if(left !== 0) {count++}
        if(right !== 0) {count++}
